...
public class GameUIInput : UIWidgetContainer
{
    ...
    static public GameUIInput current;
    ...
    [System.Obsolete("Use GameUIInput.value instead")]
	...
	void Update()
	{
        ...
        if (mKeyboard != null)
        {
            ...
            if (mKeyboard.done)
            {
                ...
                {
                    ChatInfoLogic.Instance().ChatInfoGoBack();
                }
            }

            if (TouchScreenKeyboard.visible == false && selected == true)
            {
                selected = false;
            }
        }
 ...
 (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)))
    ...
    void UpdateLabel()
    {
        if (LoudSpeakerLogic.Instance() != null && this == LoudSpeakerLogic.Instance().m_LoudSpeakerInput)
        {
            if (!LoudSpeakerLogic.Instance().UpdateRemainText())
            {
                return;
            }
        }
    ...
    void InitDefaultTextOnTellChat()
    {
        if (ChatInfoLogic.Instance() == null)
        {
            return;
        ...
        if (gameObject.name != "ChatInfoInput")
        {
            return;
        }

        if (ChatInfoLogic.Instance().GetCurChannelType() != (int)CG_CHAT.CHATTYPE.CHAT_TYPE_TELL &&
            ChatInfoLogic.Instance().GetCurChannelType() != (int)CG_CHAT.CHATTYPE.CHAT_TYPE_FRIEND)
        {
            return;
        }

        if (ChatInfoLogic.Instance().TellChatReceiverName == "" &&
            ChatInfoLogic.Instance().FriendChatReceiverName == "")
        {
            return;
        }
        ...
        if (ChatInfoLogic.Instance().GetCurChannelType() == (int)CG_CHAT.CHATTYPE.CHAT_TYPE_TELL)
        {
            mText = "/" + ChatInfoLogic.Instance().TellChatReceiverName + " ";
        }
        if (ChatInfoLogic.Instance().GetCurChannelType() == (int)CG_CHAT.CHATTYPE.CHAT_TYPE_FRIEND)
        {
            mText = "/" + ChatInfoLogic.Instance().FriendChatReceiverName + " ";
        }
    }
}
