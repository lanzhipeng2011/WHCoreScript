...
public class NotificationManager : MonoBehaviour
{
...
#if UNITY_ANDROID && (USE_PUSH || USE_LOCALPUSH)
    ...
    public static void InitLocationNotification(string msgtitle, string msgContent, DateTime fireTime,
        int notificationID, bool isRepeate = false, NotiCalendarUnit repeateInteval = NotiCalendarUnit.Day)
    {
#if UNITY_IPHONE && (USE_PUSH || USE_LOCALPUSH)
        string[] msgs = msgContent.Split('#');
        if (msgs.Length != 2)
            return;

        string[] weekData = msgs[1].Split(';');
        if (weekData.Length != 7)
            return;

        //Debug.LogError("fireTime:" + fireTime);
        
        //设置未来14天的通知
        for (int i = 0; i < 14; i++) 
        {
            DateTime tmpFireTime = new DateTime(fireTime.Ticks);
            //DateTime tmpFireTime = fireTime;//.AddDays(1);

            DayOfWeek week = tmpFireTime.DayOfWeek;
            bool isTriiger = weekData[(int)week] == "1";

            if (isTriiger) 
            {
                LocalNotification localNotification = new LocalNotification();
                localNotification.fireDate = tmpFireTime;
                localNotification.alertBody = msgs[0];
            ...
            //Debug.LogError("tmpFireTime:" + tmpFireTime + ",isTrigger:" + isTriiger + ",           content:" + msgs[0]);
        ...
        //NotificationServices.ScheduleLocalNotification (localNotification);
#elif UNITY_ANDROID && (USE_PUSH || USE_LOCALPUSH)
        GetDurationSecondTime(DateTime.Now, fireTime);
        GetIntevalSeconds(repeateInteval);
        //Debug.Log("setPush data:" + msgtitle + ",content:" + msgContent);
