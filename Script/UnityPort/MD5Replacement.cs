...
namespace UnityPortMD5
{
    public class MD5CryptoServiceProvider
    {
        ...
        private Digest CaculateMD5Value()
        {
        ...
        {
            m_byteInput = new byte[(int)inputStream.Length];
            inputStream.Read(m_byteInput, 0, (int)inputStream.Length);
            m_digest = CaculateMD5Value();
